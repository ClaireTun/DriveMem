...
# 类外部定义全局变量（多进程下每个进程独立）
BAD_INDICES_FILE = "bad_samples.log"
_RETRY_COUNT = defaultdict(int)  # 记录每个索引的重试次数
...
class B2D_VAD_Dataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        # path1 = "/disk1/zhaoxue/bev/RL/Bench2Drive/Bench2DriveZoo/data/trajs/train/" + image_paths[0].replace('/','_').replace('.jpg','.npy')
        # np.save(path1, ego_fut_trajs)         
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if data is None:               
                # with open("bad_name.txt","a+") as f:
                    # f.write(data['img_metas'].data[0]['filename'][0]+"\n")
    ...
    # def __getitem__(self, idx):
        ...
        # Note:
            # This method includes error handling to avoid training interruption.
            # Bad samples are logged to a file for later inspection.
        # """
        # if self.test_mode:
            # return self.prepare_test_data(idx)
        ...
        # while attempts < max_retries:
            # try:
                # # 尝试加载数据
                # data = self.prepare_train_data(current_idx)
                
                # # 检查数据是否有效
                # if data is None:
                    # raise ValueError(f"prepare_train_data returned None for index {current_idx}")
                
                # # 加载成功，重置重试计数
                # _RETRY_COUNT[current_idx] = 0
                # return data
                
            # except Exception as e:
                # last_error = e
                # attempts += 1
                # _RETRY_COUNT[current_idx] += 1
                
                # # 记录坏样本信息（多进程安全）
                # self._log_bad_sample(current_idx, e)
                
                # # 如果未达到最大重试次数，尝试换一个同组样本
                # if attempts < max_retries:
                    # try:
                        # # 使用 _rand_another 获取同组另一个样本
                        # new_current_idx = self._rand_another(current_idx)
                        # print(f"⚠️ 加载失败 [尝试 {attempts}/{max_retries}]: 索引 {current_idx}, 替换为新索引 {new_current_idx}")
                        # current_idx = new_current_idx
                    # except Exception as sampler_err:
                        # print(f"❌ 采样器替换失败：{sampler_err}")
                        # break
                # else:
                    # # 达到最大重试次数，抛出明确错误
                    # error_msg = (
                        # f"无法加载数据，已重试 {max_retries} 次。"
                        # f"最后失败索引：{current_idx}, 错误：{last_error}"
                    # )
                    # print(f"❌ {error_msg}")
                    # # 注意：这里选择抛出异常而非返回 dummy，确保问题被暴露
                    # raise RuntimeError(error_msg) from last_error
        
        # # 理论上不会走到这里
        # raise RuntimeError(f"未知错误：索引 {idx} 加载失败")

    def _log_bad_sample(self, idx, error):
        """记录坏样本到文件（多进程安全）"""
        try:
            # 获取样本文件路径（如果可用）
            info = self.data_infos[idx] if hasattr(self, 'data_infos') else {}
            filename = info.get('filename', 'N/A') if isinstance(info, dict) else 'N/A'
            ...
            # 写入日志文件
            with open(BAD_INDICES_FILE, 'a') as f:
                fcntl.flock(f.fileno(), fcntl.LOCK_EX)  # 加锁
                log_entry = (
                    f"[{os.path.basename(BAD_INDICES_FILE)}] "
                    f"Rank: {rank} | PID: {pid} | "
                    f"Index: {idx} | File: {filename} | "
            ...
            # 同时打印到控制台
            print(f"🚫 坏样本记录：Index {idx} | File: {filename} | Error: {error}")
            
        except Exception as log_err:
            # 日志记录失败不应影响训练
            print(f"⚠️ 日志记录失败：{log_err}")        
