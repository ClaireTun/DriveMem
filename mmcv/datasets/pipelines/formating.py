    ...
    def __call__(self, results):
        ...
        if self.with_gt:
            ...
            if self.with_label:
                ...
                # elif 'gt_names' in results:
                    # names = results['gt_names']
                    # bboxes = results['gt_bboxes_3d']

                    # # 获取底层 tensor 数据
                    # bbox_tensor = bboxes.tensor if hasattr(bboxes, 'tensor') else bboxes
                    # n_names, n_bboxes = len(names), len(bbox_tensor)

                    # # 🔑 核心修复：以实际保留的 bbox 数量为准，截断多余的 name
                    # if n_names != n_bboxes:
                        # names = names[:n_bboxes]

                    # # 生成有效掩码（此时长度已严格 = n_bboxes）
                    # valid_mask = np.array([n in self.class_names for n in names])

                    # # 同步过滤
                    # results['gt_names'] = [n for n, m in zip(names, valid_mask) if m]
                    # results['gt_bboxes_3d'] = bboxes[valid_mask]  # MMDet3D 的 Box 对象支持布尔索引

                    # # 生成 labels（纯净列表，绝不会 ValueError）
                    # if results['gt_names']:
                        # results['gt_labels'] = np.array([
                            # self.class_names.index(n) for n in results['gt_names']
                        # ], dtype=np.int64)
