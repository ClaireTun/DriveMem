...
class VADHead(DETRHead):
    ...
    def forward(self,
        ...
        # root = "/disk1/zhaoxue/bev/RL/Bench2Drive/Bench2DriveZoo/data/feats/train/"
        
        # with open('pic_train.json', 'a+') as f:
            # json.dump(img_metas[0]['filename'], f)
            # f.write('\n')
        
        # with open("pic_val.txt", 'a+', encoding='utf-8') as file:
                # file.write(img_metas[0]['filename']+'\n')

        # path1 = root + img_metas[0]['filename'][0].replace('/','_').replace('.jpg','.npy')
...
class CausalPrototypeMemory2(nn.Module):
    def __init__(self,
                 num_slots,
                 d_causal,
                 tau=0.1,
                 svd_align_weight=0.1,
                 div_weight=0.1,
                 name=""):
        """
        Causal Prototype Memory with pure gradient-based learning.
        Focuses on "typical yet hard" samples via per-sample loss weighting.
        
        Args:
            num_slots (int): Number of prototype slots.
            d_causal (int): Dimension of causal feature.
            tau (float): Temperature for prototype assignment.
            svd_align_weight (float): Weight for SVD alignment loss.
            div_weight (float): Weight for prototype diversity loss.
            name (str): Identifier (e.g., 'agent' or 'ego').
        """
        super().__init__()
        self.num_slots = num_slots
        self.d_causal = d_causal
        self.tau = tau
        self.svd_align_weight = svd_align_weight
        self.div_weight = div_weight
        self.name = name

        # Learnable prototypes
        self.prototypes = nn.Parameter(torch.randn(num_slots, d_causal) * 0.02)

    def retrieve(self, z_causal):
        """Inference-time retrieval (no gradient)."""
        scores = torch.matmul(z_causal, self.prototypes.T)
        weights = F.softmax(scores / self.tau, dim=-1)
        retrieved = torch.matmul(weights, self.prototypes)
        return retrieved

    def forward_train(self, z_causal, task_loss, U_top=None, top_percentile=0.8):
        """
        Training forward with weighted reconstruction loss to focus on:
        - Typical samples (low assignment entropy)
        - Hard samples (high task_loss)
        
        Args:
            z_causal: [N, d_causal] — causal features
            task_loss: [N] — per-sample task loss (e.g., trajectory L2)
            U_top: [d_causal, r] — top-r singular vectors (optional)
            top_percentile: float — retain top-% samples (e.g., 0.8 → top 20%)
            
        Returns:
            retrieved: [N, d_causal]
            losses: dict
        """
        ...
        # 1. Retrieve
        scores = torch.matmul(z_causal, self.prototypes.T)      # [N, K]
        weights = F.softmax(scores / self.tau, dim=-1)          # [N, K]
        retrieved = torch.matmul(weights, self.prototypes)      # [N, d]

        # 2. Compute typicality (low entropy = typical)
        epsilon = 1e-8
        entropy = -torch.sum(weights * torch.log(weights + epsilon), dim=1)  # [N]
        max_entropy = torch.log(torch.tensor(self.num_slots, dtype=torch.float, device=device))
        typicality = 1.0 - (entropy / (max_entropy + epsilon))  # [N], ∈ [0,1]

        # 3. Normalize task loss for hardness
        if task_loss.numel() > 1:
            hardness = (task_loss - task_loss.mean()) / (task_loss.std() + epsilon)
            hardness = torch.clamp(hardness, min=0.0)  # only consider above-mean hard examples
        else:
            hardness = torch.ones_like(task_loss)

        # 4. Combine: focus on "typical AND hard"
        sample_weight = typicality * hardness  # [N]

        # 5. Sparsify: only top-percentile samples contribute
        threshold = torch.quantile(sample_weight, top_percentile)
        sample_weight = (sample_weight >= threshold).float()  # binary mask, [N]

        # 6. Weighted reconstruction loss
        recon_error = (retrieved - z_causal.detach()) ** 2  # [N, d]
        weighted_recon = recon_error * sample_weight.unsqueeze(-1)  # [N, d]
        loss_recon = weighted_recon.sum() / (sample_weight.sum() + epsilon)

        # 7. SVD alignment loss (optional)
        loss_align = torch.tensor(0.0, device=device)
        if U_top is not None:
            proj = self.prototypes @ U_top.T @ U_top
            loss_align = F.mse_loss(self.prototypes, proj)

        # 8. Diversity loss (minimize prototype similarity)
        loss_div = torch.tensor(0.0, device=device)
        if self.div_weight > 0:
            proto_norm = F.normalize(self.prototypes, dim=1)
            sim_mat = proto_norm @ proto_norm.T  # [K, K]
            loss_div = (sim_mat - torch.eye(self.num_slots, device=device)).pow(2).mean()

        # 9. Total loss
        #total_loss = loss_recon + self.svd_align_weight * loss_align + self.div_weight * loss_div

        losses = {
            #f"loss_{self.name}_memory": total_loss,
            f"loss_{self.name}_recon": loss_recon,
            f"loss_{self.name}_align": loss_align * self.svd_align_weight,
            f"loss_{self.name}_div": loss_div * self.div_weight
        }

        return retrieved, losses
class CausalDisentangler(nn.Module):
    """
    改进版因果解耦模块：先升维，再解耦
    输入: z ∈ [B, input_dim]
    输出: z_causal ∈ [B, causal_dim], z_spurious ∈ [B, spurious_dim]
    """

    def __init__(self, input_dim, causal_dim=None, spurious_dim=None, hidden_ratio=2,l_o=1.0,l_inv=1.0,mask_ratio=0.2):
        """
        :param input_dim: 输入 token 的原始维度（如 256）
        :param causal_dim: 因果部分维度（默认 = input_dim）
        :param spurious_dim: 混杂部分维度（默认 = input_dim）
        :param hidden_ratio: 升维倍数（默认 2x）
        """
        super(CausalDisentangler, self).__init__()

        self.input_dim = input_dim
        self.causal_dim = causal_dim if causal_dim is not None else input_dim
        self.spurious_dim = spurious_dim if spurious_dim is not None else input_dim
        self.hidden_dim = input_dim * hidden_ratio
        self.mask_ratio = mask_ratio

        # Step 1: 升维 MLP (可加激活函数)
        self.expand_proj = nn.Sequential(
            nn.Linear(input_dim, self.hidden_dim),
            nn.ReLU(),  # 引入非线性，增强表达能力
        )

        # Step 2: 从高维空间分别投影到因果/混杂子空间
        self.W_causal = nn.Linear(self.hidden_dim, self.causal_dim, bias=False)
        self.W_spurious = nn.Linear(self.hidden_dim, self.spurious_dim, bias=False)

        # Token Aug初始化
        #self.token_aug = FreqJitter(eta=0.25, pi=1.0)
        
        self.l_o = l_o
        self.l_inv = l_inv                
                
        # 初始化
        nn.init.xavier_uniform_(self.expand_proj[0].weight)
        nn.init.xavier_uniform_(self.W_causal.weight)
        nn.init.xavier_uniform_(self.W_spurious.weight)

    def forward(self, z):
        """
        :param z: [B, input_dim]
        :return:
            z_causal: [B, causal_dim]
            z_spurious: [B, spurious_dim]
        """
        # Step 1: 升维
        z_expanded = self.expand_proj(z)  # [B, hidden_dim]

        # Step 2: 解耦
        z_causal = self.W_causal(z_expanded)      # [B, causal_dim]
        z_spurious = self.W_spurious(z_expanded)  # [B, spurious_dim]

        return z_causal, z_spurious

    def orthogonality_loss(self, z_causal, z_spurious):
        """
        强制 z_causal 和 z_spurious 特征正交
        """
        min_dim = min(z_causal.size(1), z_spurious.size(1))
        if min_dim > 0:
            #print("here=======================")
            z_c = F.normalize(z_causal[:, :min_dim], dim=-1)
            z_s = F.normalize(z_spurious[:, :min_dim], dim=-1)
            dot = (z_c * z_s).sum(dim=1)  # [B]
            loss = (dot ** 2).mean()
        else:
            loss = torch.tensor(0.0, device=z_causal.device)               
            
        return loss*self.l_o

    def invariance_loss(self, z, z_causal_original, noise_std=0.1):
        """
        因果不变性损失：对原始输入加噪，要求 z_causal 不变
        """
        # noise = torch.randn_like(z) * noise_std
        # z_perturbed = z + noise
                
        #z_perturbed = self.token_aug(z)  # z: [B, N, D]
        #z_causal_perturbed = self.forward(z_perturbed)[0]

        B, D = z.shape
        device = z.device

        # 生成随机掩码：每行独立 mask
        num_mask = int(self.mask_ratio * D)
        mask = torch.ones(B, D, device=device)
        for i in range(B):
            indices = torch.randperm(D)[:num_mask]
            mask[i, indices] = 0.0
        z_masked = z * mask

        # 前向得到扰动后的因果特征
        z_causal_perturbed = self.forward(z_masked)[0]  # [B, causal_dim]              
        loss = F.mse_loss(z_causal_original, z_causal_perturbed)
        return loss*self.l_inv

@HEADS.register_module()
class VADHeadVCA(DETRHead):
    """Head of VAD model.
    Args:
        with_box_refine (bool): Whether to refine the reference points
            in the decoder. Defaults to False.
    ...
    def __init__(self,
        ...
        super(VADHeadVCA, self).__init__(*args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.map_code_weights = nn.Parameter(torch.tensor(
            self.map_code_weights, requires_grad=False), requires_grad=False)
        ...
        # Causal Disentangler
        #self.embed_dims = 256
        self.causal_disentangler = CausalDisentangler(
            input_dim=self.embed_dims,
            causal_dim=self.embed_dims,
            spurious_dim=self.embed_dims,
            hidden_ratio=2,
            l_o=1.0,
            l_inv=1.0,
            mask_ratio=0.2
        ...
        # # Memory
        # self.agent_memory = CausalPrototypeMemory2(
            # num_slots=2048,
            # d_causal=self.embed_dims,
            # tau=0.1,
            # svd_align_weight=1.0,
            # div_weight=0.0,
            # name="agent"
        # )
        # self.ego_memory = CausalPrototypeMemory2(
            # num_slots=128,
            # d_causal=self.embed_dims,
            # tau=0.1,
            # svd_align_weight=1.0,
        ...
        # # Gating
        # self.agent_gate = nn.Linear(self.embed_dims * 2, 1)
        # self.ego_gate = nn.Linear(self.embed_dims * 2, 1)
    ...
    def forward(self,
        ...
        if self.motion_decoder is not None:
            ...
            ##############add causal##################            
            # ========= 新增：因果解耦 =========
            hs_features = hs[-1]  # [B, N_all, D]
            B, N_all, D = hs_features.shape
            hs_features_flat = hs_features.view(-1, D)  # [B*N, D]
            z_causal, z_spurious = self.causal_disentangler(hs_features_flat)
            motion_query_causal = z_causal.view(B, N_all, D)                        
            motion_query = motion_query_causal.permute(1, 0, 2)  # [A, B, D]                        
            ###################################################
            mode_query = self.motion_mode_query.weight  # [fut_mode, D]
            # [M, B, D], M=A*fut_mode
            motion_query = (motion_query[:, None, :, :] + mode_query[None, :, None, :]).flatten(0, 1)
        ...
        if self.training:
            #print("causal+++++++++++++++++++")
            L_orth = self.causal_disentangler.orthogonality_loss(z_causal, z_spurious)
            L_invar = self.causal_disentangler.invariance_loss(hs_features_flat, z_causal)
        else:
            L_orth = torch.tensor(0.0, device=hs_features.device, requires_grad=True)
            L_invar = torch.tensor(0.0, device=hs_features.device, requires_grad=True)
    ...
    def loss(self,
        ...
        loss_dict = dict()
        #####add causal loss####
        loss_dict["L_orth"] = preds_dicts["L_orth"]
        loss_dict["L_invar"] = preds_dicts["L_invar"]
        # loss from the last decoder layer
        loss_dict['loss_cls'] = losses_cls[-1]
        loss_dict['loss_bbox'] = losses_bbox[-1]
        loss_dict['loss_traj'] = loss_traj[-1]
...
class VADHeadCADM(DETRHead):
    ...
    def __init__(self,
        ...
        super(VADHeadCADM, self).__init__(*args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.map_code_weights = nn.Parameter(torch.tensor(
            self.map_code_weights, requires_grad=False), requires_grad=False)
    ...
    def forward(self,
        ...
        # ========== NEW: Memory retrieval & gating fusion ==========
        # 缓存原始 query（用于 loss 中更新 memory）
        outs = dict()
        outs["_agent_query_before_mem"] = agent_query.detach()  # [B, N_pad, D]
        outs["_ego_query_before_mem"] = ego_query.detach()      # [B, 1, D]
        outs["_agent_mask"] = agent_mask                        # [B, N_pad]

        # Memory retrieve + gate（训练/推理都做）
        B, N_pad, D = agent_query.shape
        agent_flat = agent_query.view(-1, D)
        ego_flat = ego_query.view(-1, D)

        retrieved_agent = self.agent_memory.retrieve(agent_flat)
        retrieved_ego = self.ego_memory.retrieve(ego_flat)

        # Gating fusion
        gate_agent = torch.sigmoid(self.agent_gate(torch.cat([agent_flat, retrieved_agent], dim=-1)))
        fused_agent = gate_agent * agent_flat + (1 - gate_agent) * retrieved_agent
        agent_query = fused_agent.view(B, N_pad, D)

        gate_ego = torch.sigmoid(self.ego_gate(torch.cat([ego_flat, retrieved_ego], dim=-1)))
        fused_ego = gate_ego * ego_flat + (1 - gate_ego) * retrieved_ego
        ego_query = fused_ego.view(batch, 1, D)
        #torch.Size([1, 300, 256]) torch.Size([1, 1, 256])
        #print(agent_query.shape,ego_query.shape)
        # ========================================================
    ...
    def loss_single(self,
        ...
        #####################for memory update 
        ## Step 2: 计算 per-query loss（不 reduce）
        traj_loss_raw = F.l1_loss(best_traj_preds, traj_targets, reduction='none')  # [N, fut_ts*2]
        traj_loss_per_query = (traj_loss_raw * traj_weights).sum(dim=1) / (traj_weights.sum(dim=1) + 1e-6)  # [N]
        # Step 3: reshape 回 [B, num_query]
        traj_loss_per_batch = traj_loss_per_query.view(1, 300)  # [B, num_query]
        ...
        return loss_cls, loss_bbox, loss_traj, loss_traj_cls, traj_loss_per_batch.detach() 
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox, loss_traj, loss_traj_cls, per_agent_motion_loss_list = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds, all_traj_preds,
            all_traj_cls_scores, all_gt_bboxes_list, all_gt_labels_list,
            all_gt_attr_labels_list, all_gt_bboxes_ignore_list)
        
        per_agent_motion_loss = per_agent_motion_loss_list[-1]  # [B, 300]
        ...
        # ========== MEMORY UPDATE ==========
        if self.training:                       
            # Check if per-sample losses and queries are cached
            if "_agent_query_before_mem" in preds_dicts and "_ego_query_before_mem" in preds_dicts:
                agent_query = preds_dicts["_agent_query_before_mem"]  # [B, N_pad, D]
                ego_query = preds_dicts["_ego_query_before_mem"]      # [B, 1, D]
                agent_mask = preds_dicts["_agent_mask"] # [B, N_pad]
                
                # ============ 全局 gather 所有卡的 ego ============
                if torch.distributed.is_initialized():
                    world_size = torch.distributed.get_world_size()
                    if world_size > 1:
                        # Gather ego_features: [1, D] → [world_size, D]
                        ego_features_list = [torch.zeros_like(ego_query) for _ in range(world_size)]
                        torch.distributed.all_gather(ego_features_list, ego_query)
                        ego_features_global = torch.cat(ego_features_list, dim=0)  # [N_global, D]
                        # Gather ego_loss: [1] → [N_global]
                        ego_loss_list = [torch.zeros_like(loss_dict['loss_plan_reg']) for _ in range(world_size)]
                        torch.distributed.all_gather(ego_loss_list, loss_dict['loss_plan_reg'])
                        ego_loss_global = torch.tensor(ego_loss_list)  # [N_global]

                        # Use global for memory update
                        ego_features = ego_features_global.cuda()
                        ego_loss = ego_loss_global.cuda()


                B, N_pad, D = agent_query.shape
                # Align per-agent loss to padded shape (simple: pad with 0)
                agent_loss_padded = torch.zeros(B, N_pad, device=agent_query.device)
                N_orig = per_agent_motion_loss.shape[1]
                agent_loss_padded[:, :N_orig] = per_agent_motion_loss

                # Flatten
                agent_flat = agent_query.view(-1, D)          # [B*N_pad, D]
                agent_loss_flat = agent_loss_padded.view(-1)  # [B*N_pad]
                ego_flat = ego_features.view(-1, D)              # [B, D]
                ego_loss_flat = ego_loss         # [B]

                # SVD for agent (valid only)
                with torch.no_grad():
                    valid_mask = ~agent_mask.view(-1)  # [B*N_pad]
                    if valid_mask.sum() > 0:
                        _, _, Vt_a = torch.svd(agent_flat[valid_mask])
                        r = min(50, Vt_a.shape[0])
                        #print(r,"+++++++++++++++")
                        V_top_a = Vt_a.T[:r, :]  # [r, D]
                        #V_top_a = Vt_a.T[:50, :]
                    else:
                        V_top_a = None
                        
                    _, _, Vt_e = torch.svd(ego_flat)
                    V_top_e = Vt_e.T[:10, :]

                # Update memory
                # _, agent_mem_losses = self.agent_memory.forward_train(
                    # agent_flat, agent_loss_flat, U_top=None)
                _, agent_mem_losses = self.agent_memory.forward_train(
                agent_flat, agent_loss_flat, U_top=V_top_a)
                _, ego_mem_losses = self.ego_memory.forward_train(
                    ego_flat, ego_loss_flat, U_top=V_top_e)

                # Add to total loss
                loss_dict.update(agent_mem_losses)
                loss_dict.update(ego_mem_losses)    
...
@HEADS.register_module()
class VADHeadCADM_ALL(DETRHead):
    """Head of VAD model.
    Args:
        with_box_refine (bool): Whether to refine the reference points
            in the decoder. Defaults to False.
    ...
    def __init__(self,
        ...
        super(VADHeadCADM_ALL, self).__init__(*args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.map_code_weights = nn.Parameter(torch.tensor(
            self.map_code_weights, requires_grad=False), requires_grad=False)
            ...
            if "_agent_query_before_mem" in preds_dicts and "_ego_query_before_mem" in preds_dicts:
                ...
                if torch.distributed.is_initialized():
                    ...
                    if world_size > 1:
                        ...
                        # --- 单卡逻辑 (新增逻辑) ---
                        # 单卡模式下，全局特征就是本地特征，无需 gather
                        ego_features = ego_query.cuda()
                        
                        # 确保 loss 的维度与多卡模式一致 (多卡模式下是 [N_global]，单卡最好是 [1])
                        local_loss = loss_dict['loss_plan_reg']
                        if local_loss.dim() == 0:  # 如果是标量，unsqueeze 变成 [1]
                            ego_loss = local_loss.unsqueeze(0).cuda()
                        else:
                            ego_loss = local_loss.cuda()
